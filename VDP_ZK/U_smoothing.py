...
err = 1e-6          # terminating condition for loss
bach_size = 32
...
net_dd = torch.load('Van_der_Pol_dd_Lyap_layer_2_width_15_samples_{}_lr_0.001_epoch_300.pt'.format(M))
...
levels2 = [0.128]
ax.contour(x_mesh, y_mesh, y_train.reshape(x_mesh.shape), colors='#00008B', levels=[0.012], linewidths=3, linestyles='-.', label='Predicted ROA boundary')
ax.contour(x1_vals, x2_vals, y_vals, levels=levels2, colors='#00008B',linewidths=2, label='Verified boundary')
ax.contour(x1_vals, x2_vals, y_vals_dd, colors='green', levels=[0.15], linewidths=1, linestyles='-.', label='DD-Predicted ROA boundary')
ax.contour(x1_vals, x2_vals, y_vals_dd, colors='green', levels=[0.154], linewidths=1, label='DD-Predicted ROA boundary')
ax.plot(vsol.y[0], vsol.y[1], color='red', linewidth=1,  label='Limit cycle (ROA boundary)')
# Plot the level sets
ax.set_xlabel('$x_1$')
ax.set_ylabel('$x_2$')
ax.set_ylim(-3, 3)
...
legend_handles = [
    ...
    Line2D([0], [0], color='#00008B', linewidth=1.8, linestyle='-.', label='ZK-Predicted boundary'),
    Line2D([0], [0], color='#00008B', linewidth=1.8, label='ZK-verified'),    
    Line2D([0], [0], color='green', linewidth=1.2, linestyle='-.', label='Data-Driven Predicted boundary'),
    Line2D([0], [0], color='green', linewidth=1.2, label='Data-Driven verified'),
    Line2D([0], [0], color='#AA0000', linewidth=1.2, linestyle='-.',label='ROA boundary'),
]

ax.legend(handles=legend_handles, loc='lower right')
ax.set_title('Prediction and Verification of the Region of Attraction')
plt.text(1.8, 2.25, '$(M=300^2)$', fontsize=12, color='black', va='top', ha='left')

plt.show()
fig2.savefig('ROA_pred_{}.png'.format(M), dpi=300) 
