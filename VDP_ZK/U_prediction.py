...
def exp_g(n, m, x):
    return np.cos(np.pi/high*(n*x[:, 0])+np.pi/high*(m*x[:, 1])) * np.exp(-x[:, 0]**2/4-x[:, 1]**2/4)
...
def plots(tensor, x_mesh, y_mesh, mesh_stack_test):
    ...
    ax.legend()
    ax.set_title('Level sets of $U_{ZK}$')
...
if __name__ == "__main__":
    ...
    xx = np.linspace(-3., 3., M_test_for_1d)
    yy = np.linspace(-3., 3., M_test_for_1d)
