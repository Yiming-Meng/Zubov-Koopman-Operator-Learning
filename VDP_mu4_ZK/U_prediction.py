...
def exp_g(n, m, x):
    return np.cos(np.pi/high*(n*x[:, 0])+np.pi/high*(m*x[:, 1])) * np.exp(-x[:, 0]**2/15-x[:, 1]**2/15)
...
if __name__ == "__main__":
    ...
    xx = np.linspace(low, high, M_test_for_1d)
    yy = np.linspace(low, high, M_test_for_1d)
