...
def exp_g(n, m, x):
    return np.cos(np.pi/high*(n*x[:, 0])+np.pi/high*(m*x[:, 1])) * np.exp(-x[:, 0]**2/25-x[:, 1]**2/25)
...
if __name__ == "__main__":
    ...
    xx = np.linspace(-6., 6., M_test_for_1d)
    yy = np.linspace(-6., 6., M_test_for_1d)
    ...
    if os.path.exists(U_filename):
        ...
        if overwrite.lower() == "no":
            ...
            plt.plot(vsol1.y[0], vsol1.y[1], color='red', linewidth=2, label='Approximate ROA boundary')
            plt.plot(vsol2.y[0], vsol2.y[1], color='red', linewidth=2)
            plt.plot(vsol3.y[0], vsol3.y[1], color='red', linewidth=2)
            plt.plot(vsol4.y[0], vsol4.y[1], color='red', linewidth=2)
            plt.xlim(-5.5,5.5)
            plt.ylim(-5.5,5.5)
