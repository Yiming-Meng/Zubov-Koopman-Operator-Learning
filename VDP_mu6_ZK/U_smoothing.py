...
ax.contour(x_mesh, y_mesh, y_train.reshape(x_mesh.shape), colors='#00008B', levels=[0.15], linewidths=1.5, linestyles='-.', label='Predicted ROA boundary')
#cs = ax.contour(x1_vals, x2_vals, y_vals, levels=levels2, linewidths=1.5, label='Level sets')
#ax.contour(x1_vals, x2_vals, y_vals_dd, colors='green', levels=[0.003], linewidths=1.5, linestyles='-.', label='DD-Predicted ROA boundary')
...
sc=ax.scatter(filtered_x_test[:,0], filtered_x_test[:,1], c=1-filtered_y_test, cmap='tab20c', s=2)
#fig2.colorbar(sc)
...
plt.text(2.2, 3.25, '$\mu = 6$', fontsize=12, color='black', va='top', ha='left')

plt.show()
fig2.savefig('mu6.png', dpi=300) 
